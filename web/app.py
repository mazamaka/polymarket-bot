    ...
    for pos in positions:
        ...
        if pnl_pct <= -sl_pct:
            ...
            if best_bid > 0:
                ...
                # No buyers and price near zero — unsellable, give up
                logger.warning(
                    "SL SKIP %s — no bids and cur_price=%.4f", question, cur_price
                )
                _sl_tp_triggered.add(token_id)
                _save_sl_tp_triggered(_sl_tp_triggered)
            ...
            try:
                ...
                if order_status == "matched":
                    _sl_tp_triggered.add(token_id)
                    _save_sl_tp_triggered(_sl_tp_triggered)
                    _record_live_trade(pos, sell_price, pnl_pct, "stop-loss")
                    sync_broadcast(
                        "log",
                        f"STOP-LOSS: {question} | PnL: {pnl_pct * 100:+.1f}% | Sell @ {sell_price}",
                    )
                else:
                    # Not matched — count as retry
                    ...
                    if _sl_tp_retries[token_id] >= _SL_MAX_RETRIES:
                        ...
                        _sl_tp_gave_up_ts[token_id] = time.monotonic()
                        logger.warning(
                            "SL GAVE UP after %d unmatched retries for %s",
                            _SL_MAX_RETRIES,
                            question,
                        )
                    else:
                        logger.warning(
                            "SL sell NOT matched (%d/%d) for %s — will retry",
        ...
        if pnl_pct >= tp_pct:
            ...
            if best_bid > 0:
                ...
                logger.warning(
                    "TP SKIP %s — no bids and cur_price=%.4f", question, cur_price
                )
                _sl_tp_triggered.add(token_id)
                _save_sl_tp_triggered(_sl_tp_triggered)
                _sl_tp_gave_up_ts[token_id] = time.monotonic()
            ...
            try:
                ...
                if order_status == "matched":
                    _sl_tp_triggered.add(token_id)
                    _save_sl_tp_triggered(_sl_tp_triggered)
                    _record_live_trade(pos, sell_price, pnl_pct, "take-profit")
                    sync_broadcast(
                        "log",
                    ...
                    if _sl_tp_retries[token_id] >= _SL_MAX_RETRIES:
                        ...
                        _sl_tp_gave_up_ts[token_id] = time.monotonic()
                        logger.warning(
                            "TP GAVE UP after %d unmatched retries for %s",
                            _SL_MAX_RETRIES,
                            question,
                        )
                    else:
                        logger.warning(
                            "TP sell NOT matched (%d/%d) for %s — will retry",
                            _sl_tp_retries[token_id],
                            _SL_MAX_RETRIES,
                            question,
                        )
